...
UmlWindow::UmlWindow(bool batch) : Q3MainWindow(0, "Bouml", Qt::WDestructiveClose) {
  ...
  pmstyle->insertItem("Desktop", this, SLOT(desktop_style()));
  used = TRUE;
  
#ifndef QT_NO_STYLE_PLASTIQUE
  pmstyle->insertItem("Plastique", this, SLOT(plastique_style()));
  used = TRUE;
#endif
...
void UmlWindow::save_session() {
  ...
  if (open_file(fp, QIODevice::WriteOnly, TRUE) != -1) {
    ...
    switch (style) {
    case 'd': st << "desktop_style\n"; break;
    case 'p': st << "plastique_style\n"; break;
...
void UmlWindow::read_session() {
  ...
  if (!strcmp(k, "plastique_style")) {
    plastique_style();
    k = read_keyword(st);
  }
	else if (!strcmp(k, "windows_style")) {
	  windows_style();
	  k = read_keyword(st);
	}
...
void UmlWindow::desktop_style() {
  QApplication::setStyle(desktopStyle);
  style = 'd';
}

void UmlWindow::plastique_style() {
#ifndef QT_NO_STYLE_PLASTIQUE
